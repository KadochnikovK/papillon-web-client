...
export class ImageCropper {
  /**
   * Обрабатывает и нарезает изображение согласно заданным областям
   * @param {string} base64Image - Изображение в формате base64
   * @param {Array} data - Массив объектов с координатами и размерами областей для нарезки
   * @returns {Promise<Array>} - Массив объектов с нарезанными изображениями и их тегами
   */
  async processAndCropImage(base64Image, data) {
    console.log(data)
    try {
      const img = new Image();
      img.src = base64Image;
      ...
      if (!displayCtx) {
        console.error("Ошибка: Canvas API не поддерживается");
        return [];
      }
